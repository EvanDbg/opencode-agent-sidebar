...
test("floating overlay has separate click-expanded state with local escape close", () => {
  ...
  expect(source).toContain("consumeFloatingPanelMouseDown")
  expect(source).toContain("closeFloatingExpandedOnEscape")
  expect(source).toContain("let floatingPanelFocused = false")
  expect(source).toContain("syncFloatingPanelFocus")
  expect(source).toContain("resetFloatingPanelFocus")
  expect(source).toContain('api.keybind.match("escape", event)')
...
test("expanded floating overlay renders agents and todo item lines", () => {
  ...
  expect(expanded).toContain("Esc to close")
